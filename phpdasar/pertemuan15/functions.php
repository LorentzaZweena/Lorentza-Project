<?php 
	...
	function tambah($data){
 	...
 	function upload(){
 		...
 		//check apakah gambar udh di upload / belum
 		if ($error === 4) {
 			echo "<script>alert('pilih gambar terlebih dahulu!')</script>";
 			return false;
 		}

 		//check yg diupload itu gambar / bukan
	...
	function ubah($data){
		...
		$id = $data["id"];
 		...
 		//check user piih gambar baru / gak
	...
	function registrasi($data){
		...
		//check username sudah ada / belum
		$result = mysqli_query($conn, "SELECT username FROM user WHERE username = '$username'");
		if (mysqli_fetch_assoc($result)) {
			echo "<script>alert('username udah terdaftar!')</script>";
			return false;
		}
